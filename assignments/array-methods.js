...
runners.map(function(cap) {
  allCaps.push(cap.first_name.toUpperCase());
});
...
runners.filter(function(shirt) {
  if(shirt.shirt_size === "L") {
    largeShirts.push(shirt.first_name + " " + shirt.last_name);
  }
});
...
// Problem 1. Email list of each runner.

let list = [];

runners.forEach(function(mail) {
  list.push(mail.first_name + " " + mail.last_name + " = " + mail.email);
});
